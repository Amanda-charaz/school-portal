          ...
          <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-2xl w-full max-w-lg space-y-6 border border-gray-100 dark:border-gray-700/50 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-1">
                ...
                <select className="w-full p-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 outline-none font-medium text-sm text-gray-900 dark:text-white" value={formData.role} onChange={(e) => setFormData({ ...formData, role: e.target.value, assigned_class: "", assigned_subjects: [] })}>
              ...
              {formData.role !== "admin" && (
                <div className="space-y-1">
                  <label className="text-xs font-semibold uppercase tracking-wide opacity-50">{formData.role === "student" ? "Current Class" : "Assigned Classes"}</label>
                  <input className="w-full p-3 rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 outline-none font-medium text-sm text-gray-900 dark:text-white" type="text" value={formData.assigned_class} onChange={(e) => setFormData({ ...formData, assigned_class: e.target.value })} placeholder={formData.role === "student" ? "e.g., 1A" : "e.g., 1A, 2B"} />
                </div>
              )}
              {formData.role === "teacher" && (
                <div className="bg-gray-50 dark:bg-gray-900/50 p-5 rounded-2xl border border-gray-200 dark:border-gray-700">
                  <span className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-indigo-600 dark:text-indigo-400 mb-4"><BookOpen size={16} /> Assign Subjects</span>
                  <div className="grid grid-cols-2 gap-3">
                    {subjectOptions.map(sub => (
                      <label key={sub} className="flex items-center gap-2 text-xs font-medium cursor-pointer group">
              ...
              <div className="flex gap-3 p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl text-xs font-medium text-indigo-700 dark:text-indigo-300 uppercase tracking-tight leading-relaxed">
              ...
              <div className="flex justify-end gap-4 pt-4">
                <button type="button" className="px-6 py-3 rounded-2xl font-semibold text-xs uppercase tracking-wide text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" onClick={() => setIsModalOpen(false)}>Cancel</button>
                <button type="submit" className="px-6 py-3 rounded-2xl bg-indigo-600 text-white font-semibold text-xs uppercase tracking-wide shadow-lg active:scale-95 transition-all">Commit Account</button>
              </div>
            </form>
          </div>
        </div>
